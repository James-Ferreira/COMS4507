...
const ViewRecord = (props) => {
  ...
  const fetchRecord = async () => {
    ...
    let dog = await contracts.dogAncestry.methods.getDog(microchipNumber).call();
    ...
    if (recordNumber >= dog.medicals.length || recordNumber < 0) {
        return alert.show(`Record number '${recordNumber}' does not exist.`, SEVERITY.ERROR);
    }
  ...
  return (
    <Box>
      <div style={{display: "flex", justifyContent: "flex-end", minWidth: "250px"}}>
        <Button
          variant="contained"
          color="secondary"
      ...
      {record ?
        (<>      
            ...
            <Typography variant="subtitle" align="right">{moment.unix(record.date).format("DD/MM/YYYY")}</Typography>
